...
dados['Valor_por_mes'] = dados['Valor'] + dados['Condominio'] # Cria uma nova coluna chamada "Valor_por_mes" que é a soma das colunas "Valor" e "Condominio".
dados.head()

dados['Valor_por_ano'] = dados['Valor_por_mes'] * 12 + dados['IPTU'] # Cria uma nova coluna chamada "Valor_por_ano" que é a multiplicação das colunas "Valor_por_mes" e "IPTU".
dados.head()

"""## Criando colunas categóricas"""

dados['Descricao'] = dados['Tipo'] + ' em ' + dados['Bairro'] # Cria uma nova coluna chamada "Descricao" que é a concatenação das colunas "Tipo" e "Bairro".
dados.head()
...
dados['Descricao'] = dados['Tipo'] + ' em ' + dados['Bairro'] + ' com ' + \
                                        ...
                                        # Cria uma nova coluna chamada "Descricao" que é a concatenação das colunas "Tipo", "Bairro", "Quartos", "Vagas" e "IPTU".
                                        # .astype(str) = Converte numeros inteiros em string.
dados.head()

dados['Possui_suite'] = dados['Suites'].apply(lambda x: 'Sim' if x > 0 else 'Não') # Cria uma nova coluna chamada "Possui_suite" que é a verificação se a coluna "Suites" tem valor maior que 0.
dados.head()

dados.to_csv('dados_completos_dev.csv', index=False, sep=';') # Salva o arquivo em um arquivo CSV.
                                                          # sep=';' = Separa os itens da tabela por ';'